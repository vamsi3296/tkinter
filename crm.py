...
def search_customer():
	search_customers = Tk()
	search_customers.title("Listing All the Customers")
	search_customers.iconbitmap('./Images/canvas.ico')
	...
	def edit_now(id, index):
		...
		# username_box = Entry(search_customers)
	...
	def search_now():
		...
		if selected == "Search by...":
			test = Label(search_customers, text="Please select a different option")
		...
		if not result:
			...
			for index, x in enumerate(result):
				...
				edit_button.grid(row=index , column=num+1)
				for y in x:
					searched_label = Label(search_customers, text=y)
					searched_label.grid(row=3, column=num)
					num += 1
	...
	search_button = Button(search_customers, text="search_customers", command=search_now)
...
def list_customers():
	...
	list_customer_query.title("Listing All the Customers")
	list_customer_query.iconbitmap('./Images/canvas.ico')
